...
@Autonomous(name = "RED WAREHOUS Auto", group = "Autonomous Modes")
public class RedWarehouseOp extends LinearOpMode {
    ...
    Pose2d startPose = new Pose2d(0, -62, Math.toRadians(180));
    ...
    public void runOpMode() {
        ...
        Trajectory first_level = drive.trajectoryBuilder(away_from.end())
                .lineToSplineHeading(new Pose2d(-5, -33, Math.toRadians(115)))
                .build();

        Trajectory secondthird_level = drive.trajectoryBuilder(away_from.end())
                .lineToSplineHeading(new Pose2d(-5, -39, Math.toRadians(115)))
                .build();
        ...
        if (TSM_Position == 3) {
            ...
            while (pivotMotor.getCurrentPosition() <= 500 && !isStopRequested()) {
            ...
            drive.followTrajectory(secondthird_level);
            while (pivotMotor.getCurrentPosition() <= 300 && !isStopRequested()) {
                pivotMotor.setPower(1);
            }
            pivotMotor.setPower(0);
            ElapsedTime elapsedTime = new ElapsedTime();
